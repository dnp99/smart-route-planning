...
UPDATE "nurses"
SET "email" = 'nicole@careflow.local'
WHERE "external_key" = 'default-nurse' AND "email" IS NULL;--> statement-breakpoint
UPDATE "nurses"
SET "email" = CONCAT("external_key", '@careflow.local')
WHERE "email" IS NULL;--> statement-breakpoint
UPDATE "nurses"
SET "email" = LOWER(TRIM("email"));--> statement-breakpoint
UPDATE "nurses"
SET "password_hash" = crypt('careflow-dev-password', gen_salt('bf', 12))
