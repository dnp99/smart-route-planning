...
import { archivePatientsForNurse } from "../../../../lib/patients/patientRepository";
...
const parsePatientIds = (body: unknown): string[] | null => {
  ...
  // De-dupe to keep the audit count honest.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const archivedIds = await archivePatientsForNurse(auth.nurseId, patientIds);
    void logAuditEvent({
      actorNurseId: auth.nurseId,
      action: "patients.archive",
      resourceType: "patient",
      outcome: "success",
      metadata: { requestedCount: patientIds.length, archivedCount: archivedIds.length },
      ipAddress: resolveRequestIpAddress(request),
      userAgent: resolveRequestUserAgent(request),
    });

    return NextResponse.json({ archivedIds }, { headers: corsHeaders });
  } catch (error) {
    return toErrorResponse(error, "Failed to archive clients.", corsHeaders);
  }
}
